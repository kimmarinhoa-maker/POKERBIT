...
export default function ComprovantePage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch(
          `/api/comprovante/${settlementId}/${agentId}?e=${encodeURIComponent(expiry)}&sig=${encodeURIComponent(sig)}`,
        );
    ...
    load();
  }, [settlementId, agentId, expiry, sig]);
  ...
  // ─── Compute financials (same as StatementView) ──────────────────
      ...
      <div className="max-w-lg mx-auto">
        ...
        <div className="bg-dark-900 border border-dark-700 rounded-xl p-6">
            ...
            <div className="space-y-1.5 text-sm">
              {!isAvista && (
                <div className="flex justify-between">
                  ...
                  <span className={`font-mono text-xs font-bold ${clrPrint(ganhos)}`}>
                    {formatBRL(ganhos)}
                  </span>
                </div>
              )}
              ...
              <div className="flex justify-between">
                <span className="text-dark-400 text-xs">
                  Rake Gerado <span className="text-dark-600">(informativo)</span>
                </span>
              ...
              {rbAgente > 0.01 && (
                <div className="flex justify-between">
                  <span className="text-dark-300 text-xs">
                    {isDirect ? 'RB Individual' : `RB Agente (${rbRate}% do Rake)`}
                  </span>
                  <span className={`font-mono text-xs font-bold ${isDirect ? 'text-blue-400' : 'text-purple-400'}`}>
                    {formatBRL(rbAgente)}
                  </span>
                </div>
              )}
          ...
          {ledgerEntries.length > 0 && (
            <div className="mb-4">
              <div className="flex items-center justify-between mb-2">
                <h4 className="text-[10px] text-dark-500 uppercase font-bold tracking-wider">
                ...
                {ledgerEntries.map((e) => (
                  <div key={e.id} className="flex items-center justify-between text-xs">
                    <div className="flex items-center gap-2">
                      {e.method && (
                        <span className="text-[9px] px-1.5 py-0.5 rounded bg-dark-800 text-dark-300 font-bold uppercase">
                          {e.method}
                        </span>
                      )}
                      {e.bank_account_name && (
                    ...
                    <span className={`font-mono font-bold ${
                      e.dir === 'IN' ? 'text-emerald-400' : 'text-red-400'
                    }`}>
          ...
          <div className="text-center mt-5 pt-3 border-t border-dark-800/50">
            ...
            <p className="text-[9px] text-dark-700 mt-1">
              Gerado pelo PokerBit
            </p>
          </div>
        </div>
      </div>
