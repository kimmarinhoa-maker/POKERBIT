...
import type { SettlementStatus } from '../server/types';
import { round2 } from '../server/round2';
import { normName } from '../server/normName';
import { AppError } from '../server/apiError';
  ...
  async getSettlementWithSubclubs(
    ...
    const [playersRes, agentsRes, feesRes, adjRes, carryRes, ledgerRes, agentOrgsRes] = await Promise.all([
      ...
      supabaseAdmin
        ...
        .eq('tenant_id', tenantId)
        .order('agent_name'),
      supabaseAdmin.from('fee_config').select('*').eq('tenant_id', tenantId).eq('is_active', true).eq('club_id', settlement.club_id),
      supabaseAdmin
