...
export default function PreviewStep({ preview, onNext, onBack }: PreviewStepProps) {
  ...
  const diffRows = useMemo(() => {
    ...
    return [
      ...
      { label: 'Rake Total', prev: existing.summary.total_rake_brl, next: preview.summary.total_rake_brl, isBRL: true as const },
      { label: 'GGR Total', prev: existing.summary.total_ggr_brl, next: preview.summary.total_ggr_brl, isBRL: true as const },
    ];
  ...
  const agentDiff = useMemo(() => {
    ...
    const newAgents = new Set(
      preview.available_agents.map(a => a.agent_name.toUpperCase())
    );
  ...
  const isIdenticalImport = existing && !isMerge ? (!hasDifferences && agentDiff.added.length === 0 && agentDiff.removed.length === 0) : false;
  ...
  return (
    <div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <h3 className="font-bold text-blue-300">
                Importacao adicional para esta semana
              </h3>
              <p className="text-dark-400 text-sm mt-1">
                Ja existe um fechamento para esta semana (v{existing.version}, {existing.summary.total_players} jogadores, {existing.summary.total_agents} agentes).
                Os dados desta planilha serao <span className="text-blue-400 font-medium">adicionados sem alterar</span> os dados ja importados.
              </p>
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between">
                <h3 className={`font-bold ${isIdenticalImport ? 'text-green-400' : 'text-yellow-300'}`}>
                  {isIdenticalImport
                    ? 'Planilha identica a versao atual'
                    : 'Esta planilha ja foi importada'}
                </h3>
                <button
              ...
              <p className="text-dark-400 text-sm mt-1">
                Fechamento existente: v{existing.version}, status: <span className={`font-medium ${isIdenticalImport ? 'text-green-400' : 'text-yellow-400'}`}>{existing.status}</span>
                {isIdenticalImport
              ...
              {diffOpen && (
                <div className="mt-3">
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody className="text-dark-300">
                        {diffRows.map(row => {
                          ...
                          return (
                            <tr key={row.label} className={`border-t border-dark-700/30 ${!isEqual ? 'bg-yellow-900/10' : ''}`}>
                              ...
                              <td className={`py-1.5 text-right font-mono ${isEqual ? 'text-green-400' : diff > 0 ? 'text-yellow-400' : 'text-red-400'}`}>
                  ...
                  {agentDiff.removed.length > 0 && (
                    <div className="mb-2">
                      <p className="text-red-400 text-xs font-medium mb-1">Agentes no fechamento atual que nao estao na planilha:</p>
                      <div className="flex flex-wrap gap-1">
                  ...
                  {!reimportConfirmed && (
                    <label className="flex items-center gap-2 mt-3 cursor-pointer select-none">
                      ...
                      <span className="text-dark-300 text-sm">
                        Estou ciente e quero reimportar esta semana
                      </span>
                    </label>
                  )}
                  {reimportConfirmed && (
                    <p className="text-green-400 text-xs mt-3">
                      {'\u2713'} Reimportacao confirmada — pode prosseguir.
                    </p>
                  )}
                </div>
              )}
      ...
      <div className="card mb-4">
        <div className="flex items-center justify-between">
          ...
          <div className={`px-3 py-1 rounded-full text-xs font-medium ${
            ...
            {preview.week.detected_from === 'xlsx' ? '\u{1F4CA} Do XLSX' :
             preview.week.detected_from === 'filename' ? '\u{1F4C1} Do filename' :
             '\u2699\uFE0F Fallback'}
                ...
                <table className="w-full text-xs">
                    ...
                    {pagedPlayers.map(p => {
                      ...
                      return (
                        <tr key={p.id} className="hover:bg-dark-800/30">
                          ...
                          <td className={`p-2 text-right font-mono ${p.ganhos >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                            {formatBRL(p.ganhos)}
                          </td>
                  ...
                  <tfoot>
                    <tr className="bg-dark-800/50 font-semibold text-dark-200">
                      ...
                      <td className={`p-2 text-right font-mono ${playerTotals.ganhos >= 0 ? 'text-green-400' : 'text-red-400'}`}>
      ...
      {preview.duplicate_players && preview.duplicate_players.length > 0 && (
        <div className="bg-blue-900/20 border border-blue-700/40 rounded-lg p-4 mb-4">
          <p className="text-blue-300 font-medium mb-2">
            {'\u{1F500}'} {preview.duplicate_players.length} ID{preview.duplicate_players.length !== 1 ? 's' : ''} duplicado{preview.duplicate_players.length !== 1 ? 's' : ''} {'\u2014'} valores somados automaticamente
          </p>
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
      ...
      <div className="flex gap-3">
        ...
        >
          {preview.readiness.ready
            ? (needsReimportConfirm ? '\u{1F512} Confirme a reimportacao acima' : '\u2705 Confirmar Importacao')
