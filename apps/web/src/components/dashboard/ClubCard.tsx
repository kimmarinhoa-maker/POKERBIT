...
export default function ClubCard({ clube, enabled = true, onToggle }: ClubCardProps) {
  ...
  return (
    <div className={`bg-dark-900 border border-dark-700 rounded-xl p-4 hover:border-dark-600 transition-all duration-200 flex flex-col ${!enabled ? 'opacity-40' : 'hover:scale-[1.02] cursor-pointer'}`}>
      {/* Header */}
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-2.5">
          <ClubLogo logoUrl={clube.logoUrl} name={clube.nome} size="sm" />
          <h3 className="text-lg font-extrabold tracking-wide uppercase text-dark-100">
            {clube.nome}
          </h3>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        <div>
          ...
          <p className="font-mono font-semibold text-sm text-poker-500">
            {formatCurrency(clube.rake)}
          </p>
        </div>
        <div>
          <p className="text-[10px] uppercase tracking-widest text-dark-400 mb-0.5">Resultado</p>
          <p className={`font-mono font-semibold text-sm ${resultColor}`}>
            {formatCurrency(clube.resultado)}
          </p>
        </div>
      </div>
      ...
      <div className="bg-dark-800 rounded-lg p-2.5 mb-3">
        ...
        <p className={`font-mono font-bold text-base ${acertoColor}`}>
          {formatCurrency(clube.acertoLiga)}
        </p>
      </div>
