          ...
          {(() => {
            ...
            return (
              ...
              >
                ...
                Cobrar Pendentes ({pendentes.length}){comPhone.length < pendentes.length && (
                  <span className="text-amber-400 text-[10px] ml-1">⚠ {pendentes.length - comPhone.length} sem WhatsApp</span>
                )}
              </button>
            );
          })()}
          ...
          <div className="flex flex-nowrap items-center justify-end gap-1">
            {hasMov && (
              <button
                ...
                className="text-[10px] px-2 py-1 whitespace-nowrap rounded-md transition-colors border border-poker-500/30 text-poker-400 bg-poker-500/5 hover:bg-poker-500/15"
              >
                Comprv.
              </button>
            )}
            {isDraft && hasMov && !isQuitado && (
              <button
                ...
                className={`text-[10px] px-2 py-1 whitespace-nowrap rounded-md transition-colors border font-medium ${
                  pendente > 0.01
            ...
            {hasMov && !isQuitado && pendente < -0.01 && onCobrar && (
              <button
                ...
                title={agentPhone ? `Cobrar via WhatsApp (${agentPhone})` : 'Sem WhatsApp — clique para saber como cadastrar'}
              >
                {agentPhone ? 'Cobrar' : '⚠ Cobrar'}
              </button>
            )}
          </div>
        </td>
