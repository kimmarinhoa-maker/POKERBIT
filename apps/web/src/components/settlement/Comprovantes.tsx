...
import StatementView, { type AgentFinancials } from './StatementView';
...
export default function Comprovantes({ subclub, weekStart, clubId, clubExternalId, logoUrl, settlementId, settlementStatus, onDataChange }: Props) {
  ...
  // Close WhatsApp dropdown on click outside
  useEffect(() => {
    ...
    if (waDropdownOpen) document.addEventListener('mousedown', handleClick);
          ...
          <div className="flex flex-nowrap items-center justify-end gap-1.5">
            ...
            {isDraft && hasMov && !isQuitado && (
              <button
                ...
                className={`text-[11px] px-2.5 py-1 min-w-[80px] text-center whitespace-nowrap rounded-md transition-colors border font-medium ${
                  pendente > 0.01
                ...
                {players.length > 0 ? (
                  <table className="w-full text-xs">
                    ...
                    <tbody className="divide-y divide-dark-800/30">
                      {players.map((p) => (
                        <tr key={p.external_player_id || p.nickname}>
