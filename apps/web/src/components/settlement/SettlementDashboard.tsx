  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
        <DashKpiCard label="Jogadores" value={String(t.players)} icon="users" borderColor="bg-blue-500" tooltip="Total de jogadores na semana" />
        <DashKpiCard label="Agentes" value={String(t.agents)} icon="building" borderColor="bg-purple-500" tooltip="Total de agentes ativos" />
        <DashKpiCard label="Rake Total" value={formatBRL(t.rake)} icon="game" borderColor="bg-poker-500" tooltip={`Soma do rake de todos subclubes = ${formatBRL(t.rake)}`} />
        <DashKpiCard label="GGR Total" value={formatBRL(t.ggr)} icon="target" borderColor="bg-purple-500" tooltip={`Soma do GGR de todos subclubes = ${formatBRL(t.ggr)}`} />
        <DashKpiCard
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {subclubs.map((sc) => (
          <button
            ...
            className={`bg-dark-900 border border-dark-700 rounded-xl overflow-hidden shadow-card hover:shadow-card-hover hover:-translate-y-1 transition-all duration-200 cursor-pointer text-left group ${
              sc.acertoLiga >= 0
                ? 'hover:border-poker-600/50 hover:shadow-glow-green'
                : 'hover:border-red-600/50 hover:shadow-glow-red'
            }`}
          >
            {/* Borda superior colorida */}
            <div className={`h-0.5 group-hover:h-1 transition-all duration-200 ${sc.acertoLiga >= 0 ? 'bg-poker-500' : 'bg-red-500'}`} />
            ...
            <div className="p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-dark-600 group-hover:text-poker-400 group-hover:translate-x-0.5 transition-all duration-200 text-lg">&rarr;</span>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3 pt-3 border-t border-dark-700/50">
                ...
                <div>
                  ...
                  <p
                    ...
                    title={`acertoLiga = resultado + taxas + lancamentos = ${formatBRL(sc.totals.resultado)} + ${formatBRL(sc.feesComputed.totalTaxasSigned)} + ${formatBRL(sc.totalLancamentos)}`}
                  >
    ...
    <div className="bg-dark-900 border border-dark-700 rounded-xl overflow-hidden shadow-card hover:shadow-card-hover hover:-translate-y-px transition-all duration-200 hover:border-dark-600 cursor-default">
      ...
      <div className="p-4">
        <div className="min-w-0">
          ...
          <p
            className={`text-lg font-bold font-mono truncate ${textColor || 'text-white'} ${tooltip ? 'explainable inline-block' : ''}`}
            title={tooltip}
          >
            {value}
          </p>
