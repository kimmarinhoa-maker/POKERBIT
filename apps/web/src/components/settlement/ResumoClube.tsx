...
export default function ResumoClube({ subclub, fees, weekStart, weekEnd, logoUrl }: Props) {
  ...
  return (
    <div>
      ...
      {/* ── Hidden ClubStatementView (for capture) ─────────────── */}
      <div
        ref={statementRef}
        style={{ position: 'absolute', left: '-9999px', width: '800px' }}
        className="bg-[#0f0f13] text-white p-8"
      >
        {/* 1. Cabecalho */}
        <div className="flex items-center gap-5 mb-6">
          {logoUrl ? (
            ...
            <div className="w-20 h-20 rounded-xl bg-dark-800 flex items-center justify-center shrink-0">
              <span className="text-2xl font-bold text-dark-500">{(name || '?').charAt(0).toUpperCase()}</span>
            </div>
          )}
          <div>
        ...
        {/* 2. Resumo financeiro compacto (grid 2x3) */}
        <div className="grid grid-cols-3 gap-3 mb-5">
          <StatRow label="Ganhos (P/L)" value={totals.ganhos} />
          <StatRow label="Rake" value={totals.rake} />
          <StatRow label="GGR" value={totals.ggr} />
          <StatRow label="Resultado" value={totals.resultado} highlight />
          <StatRow label="Taxas" value={feesComputed.totalTaxasSigned} />
          <StatRow label="Lançamentos" value={totalLancamentos} />
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-dark-800/80">
                  <th className="py-2 px-3 text-left text-[10px] text-dark-400 uppercase font-bold tracking-wider">Agente</th>
                  <th className="py-2 px-3 text-center text-[10px] text-dark-400 uppercase font-bold tracking-wider">Jog.</th>
                  <th className="py-2 px-3 text-right text-[10px] text-dark-400 uppercase font-bold tracking-wider">Rake</th>
                  <th className="py-2 px-3 text-right text-[10px] text-dark-400 uppercase font-bold tracking-wider">Ganhos</th>
                  <th className="py-2 px-3 text-right text-[10px] text-dark-400 uppercase font-bold tracking-wider">GGR</th>
                  <th className="py-2 px-3 text-right text-[10px] text-dark-400 uppercase font-bold tracking-wider">Resultado</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-dark-800">
                {agentGroups.map((g) => (
                  <tr key={g.agentName} className="hover:bg-dark-800/30">
                    <td className="py-2 px-3 text-dark-100 font-medium">{g.agentName}</td>
                    <td className="py-2 px-3 text-center text-dark-300">{g.players}</td>
                    <td className="py-2 px-3 text-right font-mono text-dark-300">{formatBRL(g.rake)}</td>
                    <td className={`py-2 px-3 text-right font-mono ${g.ganhos >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                      {formatBRL(g.ganhos)}
                    </td>
                    <td className="py-2 px-3 text-right font-mono text-purple-400">{formatBRL(g.ggr)}</td>
                    <td className={`py-2 px-3 text-right font-mono font-bold ${g.resultado >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
...
/** Compact stat row for the hidden ClubStatementView */
function StatRow({ label, value, highlight }: { label: string; value: number; highlight?: boolean }) {
  const clr =
    value > 0.01 ? 'text-emerald-400' : value < -0.01 ? 'text-red-400' : 'text-dark-400';
  return (
    <div className={`bg-dark-800/60 rounded-lg p-3 ${highlight ? 'ring-1 ring-amber-500/30' : ''}`}>
      <div className="text-[10px] text-dark-500 uppercase tracking-wider font-bold mb-1">{label}</div>
      <div className={`font-mono text-sm font-bold ${clr}`}>{formatBRL(value)}</div>
    </div>
  );
}
