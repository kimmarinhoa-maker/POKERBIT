...
export default function ResumoClube({ subclub, fees, weekStart, weekEnd, logoUrl, whatsappGroupLink }: Props) {
  ...
  return (
    <div>
      ...
      {showGroupModal && (() => {
        const groupMsg = buildClubMessage({
          clubName: name,
          weekStart,
          weekEnd,
          playersCount: totals.players,
          rake: totals.rake,
          profitLoss: totals.ganhos,
          resultado: totals.resultado,
          fees: {
            taxaApp: feesComputed.taxaApp,
            taxaAppPercent: fees.taxaApp,
            taxaLiga: feesComputed.taxaLiga,
            taxaLigaPercent: fees.taxaLiga,
            taxaRodeoGGR: feesComputed.taxaRodeoGGR,
            taxaRodeoApp: feesComputed.taxaRodeoApp,
            totalTaxasSigned: feesComputed.totalTaxasSigned,
          },
          lancamentos: [
            { nome: 'Overlay', valor: adjustments.overlay },
            { nome: 'Compras', valor: adjustments.compras },
            { nome: 'Security', valor: adjustments.security },
            { nome: 'Outros', valor: adjustments.outros },
          ],
          acertoLiga,
        });
        ...
        return (
          <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={(e) => { if (e.target === e.currentTarget) setShowGroupModal(false); }}>
            <div className="fixed inset-0 bg-black/70 backdrop-blur-sm" />
            <div className="relative w-full max-w-lg mx-4 animate-slide-up">
              <div className="bg-dark-900 border border-dark-700 rounded-xl p-6">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-bold text-white">Enviar Fechamento — {name}</h3>
                  <button onClick={() => setShowGroupModal(false)} className="text-dark-500 hover:text-white text-lg w-8 h-8 flex items-center justify-center rounded-full bg-dark-800/80 border border-dark-700 transition-colors">✕</button>
                </div>
                ...
                <div className="flex gap-3">
                  <button onClick={copyMsg} className="flex-1 px-4 py-2 bg-dark-800 hover:bg-dark-700 text-dark-300 rounded-lg text-sm transition-colors">
                    📋 Copiar Mensagem
                  </button>
                  {whatsappGroupLink ? (
                    <button
                      onClick={() => { copyMsg(); window.open(whatsappGroupLink, '_blank'); }}
                      className="flex-1 px-4 py-2 bg-green-600 hover:bg-green-500 text-white rounded-lg text-sm font-medium transition-colors"
                    >
                      💬 Abrir Grupo
                    </button>
                  ) : (
                    <button disabled className="flex-1 px-4 py-2 bg-dark-800 text-dark-500 rounded-lg text-sm cursor-not-allowed">
                      Cadastre o grupo na Config
                    </button>
                  )}
                </div>
              </div>
