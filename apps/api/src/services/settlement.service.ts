...
export class SettlementService {
  ...
  // ─── Batch dashboard summaries (lightweight) ────────────────────
  // Returns only dashboardTotals for multiple settlements in a single call.
  // Used by the dashboard to build charts without fetching full breakdown × N.
  async getDashboardBatch(tenantId: string, settlementIds: string[]) {
    ...
    // Fetch aggregated player metrics for all settlements in one query
    const { data: playerAggs } = await supabaseAdmin
      .from('player_week_metrics')
      .select('settlement_id, winnings_brl, rake_total_brl, ggr_brl, rb_value_brl')
      .in('settlement_id', settlementIds);

    // Fetch aggregated agent metrics (for agent count + commission)
    const { data: agentAggs } = await supabaseAdmin
      .from('agent_week_metrics')
      .select('settlement_id, commission_brl')
      .in('settlement_id', settlementIds);

    // Fetch fees per settlement
    const weekStarts = [...new Set(settlements.map((s) => s.week_start))];
    const { data: feeConfigs } = await supabaseAdmin
      .from('fee_configs')
      .select('org_id, taxa_app, taxa_liga, taxa_rodeo_ggr, taxa_rodeo_app')
      .eq('tenant_id', tenantId);

    // Fetch subclub orgs for fee lookup
    ...
    // Fetch adjustments (club_adjustments)
    const { data: adjustments } = await supabaseAdmin
      .from('club_adjustments')
      .select('org_id, value')
      .eq('tenant_id', tenantId);

    // Build a fee map: subclub_id -> fee config
    const feeMap = new Map<string, any>();
    for (const fc of feeConfigs || []) {
      feeMap.set(fc.org_id, fc);
    }

    // Build results
    const results = settlements.map((s) => {
      const players = (playerAggs || []).filter((p) => p.settlement_id === s.id);
      const agents = (agentAggs || []).filter((a) => a.settlement_id === s.id);

      const ganhos = round2(players.reduce((acc, p) => acc + Number(p.winnings_brl || 0), 0));
      const rake = round2(players.reduce((acc, p) => acc + Number(p.rake_total_brl || 0), 0));
      const ggr = round2(players.reduce((acc, p) => acc + Number(p.ggr_brl || 0), 0));
      const rbTotal = round2(players.reduce((acc, p) => acc + Number(p.rb_value_brl || 0), 0));
      const resultado = round2(ganhos + rake + ggr);

      // Simplified fee calculation per subclub
      const settlementSubclubs = (subclubs || []).filter((sc) => sc.parent_id === s.club_id);
      let totalTaxas = 0;
      for (const sc of settlementSubclubs) {
        const fee = feeMap.get(sc.id);
        if (!fee) continue;
        const scPlayers = players.filter((p: any) => {
          // We don't have subclub_id on player_week_metrics directly,
          // so we approximate from the total
          return true;
        });
        // Use settlement-level totals per subclub fee rates
        const scGanhos = ganhos / (settlementSubclubs.length || 1);
        const scGGR = ggr / (settlementSubclubs.length || 1);
        totalTaxas += (Math.abs(scGanhos) * (Number(fee.taxa_app || 0) / 100))
          + (Math.abs(scGanhos) * (Number(fee.taxa_liga || 0) / 100))
          + (Math.abs(scGGR) * (Number(fee.taxa_rodeo_ggr || 0) / 100))
          + (Math.abs(scGGR) * (Number(fee.taxa_rodeo_app || 0) / 100));
      }
      totalTaxas = round2(totalTaxas);
      const totalTaxasSigned = round2(-totalTaxas);

      // Adjustments total
      const adjTotal = round2((adjustments || [])
        .filter((a) => settlementSubclubs.some((sc) => sc.id === a.org_id))
        .reduce((acc, a) => acc + Number(a.value || 0), 0));

      const acertoLiga = round2(resultado + totalTaxasSigned + adjTotal);

      return {
        settlement: { id: s.id, week_start: s.week_start, status: s.status },
        dashboardTotals: {
    ...
    results.sort((a, b) => a.settlement.week_start.localeCompare(b.settlement.week_start));
    return results;
  }
